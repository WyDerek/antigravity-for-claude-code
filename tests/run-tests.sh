...
check "flash tier -> correct model string" 0 "$rc" "Gemini 3.8 Flash (High)" "$out"
...
check "explicit --tier overrides userConfig" 0 "$rc" "Gemini 3.8 Flash (High)" "$out"
...
check "explicit --tier beats default_model" 0 "$rc" "Gemini 3.8 Flash (High)" "$out"
...
check "invalid userConfig tier -> falls back to flash" 0 "$rc" "Gemini 3.8 Flash (High)" "$out"
...
check "bin/agy-set-model forwards to the wrapper (no CLAUDE_PLUGIN_ROOT)" 0 "$rc" "$(slug_of "$DEF_FLASH")" "$out"

# Derived from the same tier default the stub's model list is built from (see
# STUB_MODELS above) — hardcoding a slug here desynced these tests when the flash
# default moved and the slug stopped existing in the stub's list.
SET_MODEL_SLUG="$(slug_of "$DEF_FLASH")"
...
out=$(AGY_SETTINGS_FILE="$SETTINGS_TMP" "$SET_MODEL" "$SET_MODEL_SLUG" 2>&1); rc=$?
check "set by exact slug -> ok" 0 "$rc" "$SET_MODEL_SLUG" "$out"

out=$(AGY_SETTINGS_FILE="$SETTINGS_TMP" "$SET_MODEL" --current 2>/dev/null); rc=$?
check "--current reflects the write" 0 "$rc" "$SET_MODEL_SLUG" "$out"

out=$(python3 -c 'import json; print(json.load(open("'"$SETTINGS_TMP"'"))["model"])' 2>&1)
check "settings.json actually contains the new model" 0 "$?" "$SET_MODEL_SLUG" "$out"
