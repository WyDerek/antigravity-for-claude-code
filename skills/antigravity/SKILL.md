...
Those plugin options only steer `agy-delegate.sh` calls — they do not touch agy's own
persisted default (`~/.gemini/antigravity-cli/settings.json`'s `"model"` field), which is
what an older agy silently falls back to (see below) and what plain `agy` runs use. Change
that one with `agy-set-model <name>` / `/antigravity:set-model`, which validates against a
live `agy models` call instead of a hardcoded name.
...
- **Pass `--yolo`.** On 1.1.3+ the subagent tools need permission that headless mode
  ...
  `define_subagent`/`invoke_subagent`, and not web search, URL reads (agy 1.1.28+) or Vertex AI Search.)
